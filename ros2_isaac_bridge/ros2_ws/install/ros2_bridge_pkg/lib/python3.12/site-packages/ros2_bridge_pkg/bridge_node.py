...
class BridgeNode(Node):
    def __init__(self):
        ...
        self.sequence_rx_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.sequence_rx_sock.bind((SEQUENCE_IP, SEQUENCE_PORT))
        self.sequence_rx_sock.setblocking(False)
    ...
    def timer_callback(self):
        ...
        # Новый блок sequence
        try:
            data, _ = self.sequence_rx_sock.recvfrom(4096)
            msg_in = json.loads(data.decode("utf-8"))

            seq = msg_in.get("sequence", [])
            out = Int32MultiArray()
            out.data = [int(x) for x in seq]
            ...
            self.get_logger().error(f"sequence receive error: {e}")
